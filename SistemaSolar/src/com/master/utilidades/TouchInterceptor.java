...
public class TouchInterceptor extends ListView {
	...
	// Planeta seleccionado para arrastrar, posición y nombre
	public int seleccionadoInt = 0;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (mDragListener != null || mDropListener != null) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				// Se define la separacion horizontal, a la izquierda se
				// arrastra, a la derecha se pincha
				int separacion;
				// Para guardar el ancho de la pantalla
				float widthPantalla;

				// Se coge el id del planeta seleccionado para arrastrar
				seleccionadoInt = itemnum;

				if (itemnum == AdapterView.INVALID_POSITION) {
					break;
				}

				// definimos cada item como ImagenView, que es el objeto que
				// utilizamos en nuestra lista
				ImageView item = (ImageView) getChildAt(itemnum
				...
				// Se obtiene el número de pixeles del ancho la pantalla para
				// separar en la mitad del botón
				widthPantalla = getContext().getResources().getDisplayMetrics().widthPixels;
				separacion = (int) (widthPantalla / 2);
	...
	private void startDragging(Bitmap bm, int x, int y) {
		...
		// Vemos la clase que está utilizando esta funcionalidad para recuperar
		// su lista actual
		// De esa lista se obtiene el nombre del planeta que se va a arrastrar
		// actualmente
		String clase = context.getClass().getName();
		if (clase.endsWith("OrdenacionDistancia")) {
			// Se obtiene el nombre del planeta que se va a arrastrar
			OrdenacionDistancia ordenacion = (OrdenacionDistancia) context;
			seleccionadoString = ordenacion.getTouchLista().getAdapter()
					.getItem(seleccionadoInt).toString();
		}
		if (clase.endsWith("OrdenacionTamano")) {
			// Se obtiene el nombre del planeta que se va a arrastrar
			OrdenacionTamano ordenacion = (OrdenacionTamano) context;
			seleccionadoString = ordenacion.getTouchLista().getAdapter()
					.getItem(seleccionadoInt).toString();
		}
	...
	private void dragView(int x, int y) {
		...
		// se cambia la imagen del planeta que se está arrastrando para poner
		// fondo naranja
		mDragView = imagenArrastrada(seleccionadoString, mDragView);
	...
	public ImageView imagenArrastrada(String arrastrado, ImageView imagen) {

		if (arrastrado.equals("Mercurio")) {
			imagen.setImageResource(R.drawable.lista_mercurio_n);
		} else if (arrastrado.equals("Venus")) {
			imagen.setImageResource(R.drawable.lista_venus_n);
		} else if (arrastrado.equals("Tierra")) {
			imagen.setImageResource(R.drawable.lista_tierra_n);
		} else if (arrastrado.equals("Marte")) {
			imagen.setImageResource(R.drawable.lista_marte_n);
		} else if (arrastrado.equals("Jupiter")) {
			imagen.setImageResource(R.drawable.lista_jupiter_n);
		} else if (arrastrado.equals("Saturno")) {
			imagen.setImageResource(R.drawable.lista_saturno_n);
		} else if (arrastrado.equals("Urano")) {
			imagen.setImageResource(R.drawable.lista_urano_n);
		} else if (arrastrado.equals("Neptuno")) {
			imagen.setImageResource(R.drawable.lista_neptuno_n);
		} else
			imagen.setImageResource(R.drawable.lista_n);
